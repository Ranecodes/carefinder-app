...
const state = ref("");
const city = ref("");
const category = ref("public"); // Initialize with a default value if needed
const phone = ref("");
const email = ref("");
...
const addHospital = async () => {
  console.log("addHospital function triggered");
  if (name.value && city.value && state.value) {
    ...
    const { data, error } = await supabase
      ...
      .upsert([newHospital]);
    if (error) {
      console.error("Error adding hospital to supaBase", error);
    } else {
      hospitals.value.push(data[0]);
    }
    clearForm();
  }
};
...
const handleImageUpload = (event) => {
  ...
  // Convert the FileList object to an array and store in imageFiles
  imageFiles.value = Array.from(selectedFiles);
};
</script>
